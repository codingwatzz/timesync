...
async function scanicCrop(inputPath) {
  ...
  const flaechenanteil = (w * h) / (img.width * img.height);
  if (flaechenanteil < MIN_FLAECHENANTEIL) {
    // Erkennung mit unplausibel kleiner Flaeche - vermutlich eine Fehlerkennung (z.B.
    // Schattenwurf oder eine kleine interne Box statt des echten Blattrands), real
    // aufgetreten bei 3 von 6 August-Belegen. Lieber das Originalbild unveraendert
    // weiterverwenden als einen falschen Zuschnitt riskieren.
    return null;
  }
