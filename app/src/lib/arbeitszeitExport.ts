...
const ROT = 'FFC00000';
const ROT_FUELLUNG = 'FFFCE4E4';
const GRUEN = 'FF006100';
const GRUEN_FUELLUNG = 'FFE4F7E4';
const BLAU = 'FF1F6FB2';
const GRAU_FUELLUNG = 'FFF2F2F2';
...
function faerbeAbweichung(cell: ExcelJS.Cell, minuten: number): void {
  if (minuten > 0) {
    ...
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GRUEN_FUELLUNG } };
  } else if (minuten < 0) {
    cell.font = { color: { argb: ROT }, bold: true };
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ROT_FUELLUNG } };
  }
}
...
export async function buildArbeitszeitXlsx(year: number, month: number, entries: Record<string, TagesEintrag>): Promise<Blob> {
  ...
  function schreibeWochenZwischensumme() {
    ...
    row.eachCell((cell) => {
      cell.font = { ...(cell.font ?? {}), italic: true, bold: true };
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GRAU_FUELLUNG } };
    });
  ...
  function schreibeKennzahl(label: string, wert: string, faerben?: number) {
    const row = ws.getRow(rowIdx++);
    row.getCell(1).value = label;
    row.getCell(1).font = { bold: true };
    row.getCell(3).value = wert;
    row.getCell(3).font = { bold: true };
    if (faerben !== undefined) faerbeAbweichung(row.getCell(3), faerben);
  }

  ws.getRow(rowIdx++).getCell(1).value = 'ZUSAMMENFASSUNG';
  ws.getCell(`A${rowIdx - 1}`).font = { bold: true, size: 12 };
  schreibeKennzahl('Gesamt-IST', fmtHHMM(gesamtIst));
  schreibeKennzahl('Gesamt-SOLL', fmtHHMM(gesamtSoll));
  schreibeKennzahl('Gesamt-EXTRA', fmtHHMMSigned(gesamtExtra), gesamtExtra);
  schreibeKennzahl('Gesamt-EXTRA (%)', `${gesamtProzent >= 0 ? '+' : ''}${gesamtProzent.toFixed(1)}%`, gesamtExtra);
  rowIdx++;
  schreibeKennzahl('Homeoffice-Quote', arbeitstageGesamt > 0 ? `${((homeofficeTage / arbeitstageGesamt) * 100).toFixed(0)}% (${homeofficeTage} von ${arbeitstageGesamt} Arbeitstagen)` : '–');
  rowIdx++;
  schreibeKennzahl('Arbeitstage', String(gesamtProTyp.A));
  schreibeKennzahl('Urlaubstage', String(gesamtProTyp.U));
  schreibeKennzahl('Krankheitstage', String(gesamtProTyp.K));
  schreibeKennzahl('Gleitfreitage', String(gesamtProTyp.G));
